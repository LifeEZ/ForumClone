"""Load demo seed content into the database.
...
SEED_POSTS = [
    {
        ...
        "content": (
            "Pure black (#000000) causes eye strain because of extreme contrast with white text. "
            "Use a tinted dark color instead — like a dark forest green — for a softer, "
            "more readable UI."
        ),
        "score": 330,
        "comment_count": 45,
    ...
    },
    {
        ...
        "content": (
            "Some mixes are mastered differently for vinyl. "
            "What records do you think benefit most from the format?"
        ),
        "score": 845,
        "comment_count": 128,
        "days_ago": 3,
    ...
    },
    {
        ...
        "content": (
            "Component-level breakpoints beat viewport-only media queries for reusable UI. "
            "Worth learning if you have not tried them yet."
        ),
        "score": 1197,
        "comment_count": 32,
        "days_ago": 4,
...
async def seed() -> None:
    async with async_session_factory() as session:
        ...
        print(f"Seeded {len(SEED_USERS)} users, {len(SEED_COMMUNITIES)} communities, {len(SEED_POSTS)} posts.")
